...
# ================== RELATIONSHIP WIZARD (inline keyboard) ==================
REL_WIZ: Dict[Tuple[int,int], Dict[str, Any]] = {}  # (chat_id, actor_tgid) -> {target_uid, target_tgid, y?, m?}

def _rel_key(chat_id: int, actor_tgid: int) -> Tuple[int,int]:
    return (chat_id, actor_tgid)

def _rel_set(chat_id: int, actor_tgid: int, **kw):
    ctx = REL_WIZ.get(_rel_key(chat_id, actor_tgid), {})
    ctx.update(kw)
    REL_WIZ[_rel_key(chat_id, actor_tgid)] = ctx
    return ctx

def _rel_pop(chat_id: int, actor_tgid: int) -> Optional[Dict[str, Any]]:
    return REL_WIZ.pop(_rel_key(chat_id, actor_tgid), None)

def _year_kb(start_year: int | None = None) -> list[list[InlineKeyboardButton]]:
    y = jalali_now_year()
    base = start_year or y
    years = list(range(base, base-16, -1))
    rows = []
    for i in range(0, len(years), 4):
        rows.append([InlineKeyboardButton(fa_digits(str(yy)), callback_data=f"rel:y:{yy}") for yy in years[i:i+4]])
    rows.append([InlineKeyboardButton("سال‌های قدیمی‌تر", callback_data=f"rel:yp:{base-16}")])
    return rows

def _month_kb(y: int) -> list[list[InlineKeyboardButton]]:
    months = list(range(1,13))
    rows = []
    for i in range(0, 12, 4):
        rows.append([InlineKeyboardButton(fa_digits(str(mm)), callback_data=f"rel:m:{y}-{mm}") for mm in months[i:i+4]])
    return rows

def _day_kb(y: int, m: int) -> list[list[InlineKeyboardButton]]:
    try:
        mdays = jalali_month_len(y, m)
    except Exception:
        mdays = 31 if m<=6 else (30 if m<=11 else 29)
    days = list(range(1, mdays+1))
    rows = []
    for i in range(0, len(days), 7):
        rows.append([InlineKeyboardButton(fa_digits(str(dd)), callback_data=f"rel:d:{y}-{m}-{dd}") for dd in days[i:i+7]])
    return rows
...
async def open_relation_wizard_by_uid(update: Update, context, target_user_id: int):
    with SessionLocal() as s:
        ...
        if not target:
            await reply_temp(update, context, "کاربر پیدا نشد. دوباره تلاش کن.", keep=True); return
        if target.tg_user_id == me.tg_user_id:
            await reply_temp(update, context, "نمی‌تونی با خودت رابطه ثبت کنی."); return
        _rel_set(g.id, me.tg_user_id, target_uid=target.id, target_tgid=target.tg_user_id)
    rows = _year_kb()
    ...
    with SessionLocal() as s:
        ...
        if PAT_GROUP["relation_any"].match(text):
            ...
            if not target_user:
                ...
                if uname:
                    target_user = s.execute(select(User).where(User.chat_id == g.id, func.lower(User.username) == (uname or '').lower())).scalar_one_or_none()
                elif did:
                    try:
                        tid = int(did)
