...
def resolve_token_from_message(msg):
    # Prefer @username if exists, else numeric id
...
PAT_REL_SET_REPLY1 = re.compile(r"^رل\s+(@[\w_]+|\d+)$")
PAT_REL_SET_REPLY2 = re.compile(r"^(@[\w_]+|\d+)\s+رل$")
PAT_REL_DEL_REPLY1 = re.compile(r"^حذف\s+رل\s+(@[\w_]+|\d+)$")
PAT_REL_DEL_REPLY2 = re.compile(r"^(@[\w_]+|\d+)\s+حذف\s+رل$")
...
async def on_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with Session(engine) as session:
        ...
        elif PAT_REL_SET.match(text) or PAT_REL_SET_REPLY1.match(text) or PAT_REL_SET_REPLY2.match(text): await handle_rel_set(update, context, session, user, text)
        elif PAT_REL_DEL.match(text) or PAT_REL_DEL_REPLY1.match(text) or PAT_REL_DEL_REPLY2.match(text): await handle_rel_del(update, context, session, user, text)
...
async def handle_rel_set(update, context, session, actor, text):
    ...
    if m:
        ...
        # reply-based fallbacks
        if PAT_REL_SET_REPLY1.match(text):
            if not (update.message and update.message.reply_to_message):
                return await update.message.reply_text("برای این فرمت باید روی پیامِ طرف مقابل ریپلای کنی.")
            tok1 = resolve_token_from_message(update.message.reply_to_message)
            tok2 = PAT_REL_SET_REPLY1.match(text).group(1)
        elif PAT_REL_SET_REPLY2.match(text):
            if not (update.message and update.message.reply_to_message):
                return await update.message.reply_text("برای این فرمت باید روی پیامِ طرف مقابل ریپلای کنی.")
            tok1 = PAT_REL_SET_REPLY2.match(text).group(1)
            tok2 = resolve_token_from_message(update.message.reply_to_message)
        else:
            return
...
async def handle_rel_del(update, context, session, actor, text):
    ...
    if m:
        ...
        if PAT_REL_DEL_REPLY1.match(text):
            if not (update.message and update.message.reply_to_message):
                return await update.message.reply_text("برای این فرمت باید روی پیام ریپلای کنی.")
            tok1 = resolve_token_from_message(update.message.reply_to_message)
            tok2 = PAT_REL_DEL_REPLY1.match(text).group(1)
        elif PAT_REL_DEL_REPLY2.match(text):
            if not (update.message and update.message.reply_to_message):
                return await update.message.reply_text("برای این فرمت باید روی پیام ریپلای کنی.")
            tok1 = PAT_REL_DEL_REPLY2.match(text).group(1)
            tok2 = resolve_token_from_message(update.message.reply_to_message)
        else:
            return
    u1 = resolve_token_to_user(session, tok1); u2 = resolve_token_to_user(session, tok2)
