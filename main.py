...
def _jalali_to_gregorian(y: int, m: int, d: int) -> tuple[int,int,int]:
    # lightweight converter (based on jalaali algorithms)
    jy = y - 979
...
async def on_group_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # birthday: "ثبت تولد YYYY/MM/DD" (admin via reply, or self)
    m = re.match(r"^ثبت\s*تولد\s+(\d{4}[-/]\d{1,2}[-/]\d{1,2})$", text)
    if m:
        dstr = m.group(1)
        d = parse_date_fa_or_en(dstr)
        if not d:
            await reply_temp(update, context, "تاریخ نامعتبر است. مثال: 1377/06/08"); return
        with SessionLocal() as s_b:
            g = ensure_group(s_b, update.effective_chat)
            if update.message.reply_to_message and is_group_admin(s_b, g.id, update.effective_user.id):
                target = upsert_user(s_b, g.id, update.message.reply_to_message.from_user)
            else:
                target = upsert_user(s_b, g.id, update.effective_user)
            target.birthday = d
            s_b.commit()
            who = "خودت" if target.tg_user_id==update.effective_user.id else f"{mention_of(target)}"
            await reply_temp(update, context, f"🎂 تولد {who} ثبت شد: {fmt_date_fa(d)}", parse_mode=ParseMode.HTML)
        return

    # relationship quick start with reply: "شروع رابطه YYYY/MM/DD" or "ثبت رل YYYY/MM/DD"
    m = re.match(r"^(?:شروع\s*رابطه|ثبت\s*رل)\s+(\d{4}[-/]\d{1,2}[-/]\d{1,2})$", text)
    if m and update.message.reply_to_message:
        dstr = m.group(1)
        d = parse_date_fa_or_en(dstr)
        if not d:
            await reply_temp(update, context, "تاریخ نامعتبر است. مثال: 1404/02/22"); return
        with SessionLocal() as s_r:
            g = ensure_group(s_r, update.effective_chat)
            me = upsert_user(s_r, g.id, update.effective_user)
            target = upsert_user(s_r, g.id, update.message.reply_to_message.from_user)
            # create or update relationship
            rel = s_r.execute(select(Relationship).where(Relationship.chat_id==g.id).where(
                ((Relationship.user1_id==me.id) & (Relationship.user2_id==target.id)) |
                ((Relationship.user1_id==target.id) & (Relationship.user2_id==me.id))
            )).scalar_one_or_none()
            if not rel:
                rel = Relationship(chat_id=g.id, user1_id=min(me.id, target.id), user2_id=max(me.id, target.id), start_date=d, active=True)
                s_r.add(rel)
            else:
                rel.start_date = d
                rel.active = True
            s_r.commit()
            await reply_temp(update, context, f"💞 شروع رابطه ثبت شد: {mention_of(me)} + {mention_of(target)} — {fmt_date_fa(d)}", parse_mode=ParseMode.HTML)
        return
