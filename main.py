...
async def open_group_admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    rows = [
        [InlineKeyboardButton("👑 ثبت من به عنوان مالک گروه", callback_data="ga:setowner:self")],
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "ga:setowner:self":
        with SessionLocal() as s:
            ...
            if g:
                g.owner_user_id = update.effective_user.id; s.commit()
        return await q.message.edit_text("✅ شما به عنوان مالک گروه ثبت شدید.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ بستن", callback_data="close")]]))
    if data == "ga:wipe":
        with SessionLocal() as s:
            g = s.get(Group, q.message.chat.id) if q.message.chat else None
            if g:
